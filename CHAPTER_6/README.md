...
### 2.4 web3 API를 사용해 온라인 데이터 액세스하기

- web3 API를 사용한 이벤트 로그 처리
![image](https://user-images.githubusercontent.com/68188768/168425134-b6379018-fac2-44b4-befc-d5b23e81b1db.png)

</br>

## 오프체인 데이터 : 외부 데이터 소스

온체인 데이터와의 차이점은 오프체인 데이터의 타입과 사용은 블록체인 프로토콜이 결정하지 않는다는 것이다. 오프체인 데이터의 경우 더 넓은 시스템의 비블록체인 부분으로 사용한다. 어떤 것이든 오프체인 데이터가 될 수 있으며 데이터 소스의 타입과 포맷은 제한이 없다. 애플리케이션 종속적이고 일반적인 데이터베이스다.

![image](https://user-images.githubusercontent.com/68188768/168425316-a3acad99-8b4f-4569-8ead-a1e3fe11dd68.png)

스마트 컨트랙트는 온체인ㅇ 저장을 쉽게하므로 오프체인에서 무엇이 일어났는지를 쉽게 파악하기 위한 함수와 데이터만을 스마트 컨트랙트에 담도록 설계해야 한다. 즉, 스마트 컨트랙트는 규칙의 강제, 준수, 규정, 출처, 리얼타임 알림을 위한 로그, 타임스탬프 활동 정보와 오프라인 오퍼레이션에 대한 메시지에 필요한 함수와 데이터만을 포함하도록 설계한다.
